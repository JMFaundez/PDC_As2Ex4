	...
	int main(int argc, char* argv[]){
	  ...
	  int Navg = 50;
	  double start_time;
	  double run_time[Navg];
	  double avgtime;
	  double stdtime2;
	  double stdtime;
	  int nthr[10] = {1, 2, 4, 8, 12, 16, 20, 24, 28, 32};
	  for (int i=0;i<10;i++) {
	  omp_set_num_threads(nthr[i]);
	  for (int k=0;k<Navg;k++){
	  	...
	  	// stop timer
	  	run_time[k] = omp_get_wtime() - start_time;
	  ...
	  for (int i=1;i<Navg;i++){
	  	  avgtime = avgtime + run_time[i]/(Navg -1); 
	  }
	  stdtime2 = 0;
	  for (int i=1;i<Navg;i++){
	  	  stdtime2 = stdtime2 + (run_time[i]-avgtime)*(run_time[i]-avgtime)/(Navg -2); 
	  }
	  stdtime = sqrt(stdtime2);

	  printf("NTh = %d, avg: %f [s], std: %f [s]\n",nthr[i],avgtime,stdtime);
	  }
	  // print the results of the DFT
      #ifdef DEBUG
